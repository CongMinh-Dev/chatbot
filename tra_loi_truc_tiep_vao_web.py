...
async def chat(request: dict = Body(...)):
    ...
    # 1. TÌM VÀ GỘP CÁC CÂU SPAM LIÊN TIẾP CỦA KHÁCH Ở CUỐI HỘI THOẠI
    user_spam_contents = []
    actual_history = []
    # Duyệt ngược từ cuối mảng tin nhắn lên
    idx = len(messages) - 1
    while idx >= 0 and messages[idx]['role'] == 'user':
        # Chèn vào đầu mảng để giữ đúng thứ tự khách nhắn
        user_spam_contents.insert(0, messages[idx]['content'])
        idx -= 1
        
    # Câu hỏi mới nhất chính là sự kết hợp của tất cả các câu spam liền nhau ở cuối
    latest_question = " ".join(user_spam_contents)
    # Phần còn lại thực sự phía trước chính là lịch sử chat cũ
    all_history = messages[:idx + 1]

    # 2. TRÍCH XUẤT LỊCH SỬ CHAT THỰC TẾ (Đã loại bỏ cụm spam cuối)
    user_history = [m for m in all_history if m['role'] == 'user']
    bot_history = [m for m in all_history if m['role'] in ['assistant', 'bot']]
    
    # Lấy 2 câu trả lời gần nhất của bot và 2 câu hỏi trước đó của khách để làm ngữ cảnh
    selected_history = bot_history[-2:] + user_history[-2:]
    
    history_text = ""
    for m in all_history:
        if m in selected_history:
            role_name = "Khách hàng" if m['role'] == 'user' else "Bot bán hàng"
            history_text += f"{role_name}: {m['content']}\n"

    start_time = time.perf_counter()
    
    # BƯỚC 1: Gọi phân tích Re-phrase (Lúc này latest_question đã được gộp đẹp đẽ)
    ...
    if target == "woocommerce":
        ...
        print(f"dữ liệu từ woo + chỉnh sửa đưa vào llm: {api_context}")
