    ...
    **kwargs: Any,
    ...
    if keyword_extraction:
        if base_url and "groq" in base_url.lower():
            # Groq Llama 3.3 chưa hỗ trợ json_schema, dùng json_object thông thường
    ...
    try:
        ...
        # sửa
        if "response_format" in kwargs and (not base_url or "groq" not in base_url.lower()):
...
async def openai_complete(
    ...
    **kwargs,
    ...
    # Lấy model name từ cấu hình toàn cục của LightRAG
    model_name = kwargs["hashing_kv"].global_config["llm_model_name"]
    
    # ÉP ĐƯỜNG TRUYỀN SANG GROQ CHO CÁC MODEL LLAMA CỦA GROQ
    if "llama" in model_name.lower():
        kwargs["base_url"] = "https://api.groq.com/openai/v1"
        kwargs["api_key"] = os.environ.get("OPENAI_API_KEY")
        
        # CHẶNG 1: SỬA LỖI TRÍCH XUẤT TỪ KHÓA ĐỂ RA ĐÚNG JSON
        if keyword_extraction:
            keyword_extraction = False 
            kwargs["response_format"] = {"type": "json_object"}
            prompt += (
                "\n\nYou must respond with a VALID JSON object containing exactly two keys: "
                '"high_level_keywords" (list of strings) and "low_level_keywords" (list of strings).'
                "\nDo not include any explanation or markdown format outside the JSON."
            )
            
        # CHẶNG 2: ÉP LLAMA 3.3 ĐỌC ĐÚNG CONTEXT ĐỒ THỊ (TRIỆT TIÊU BỊA ĐẶT CHIM/CÁ)
        else:
            context_data = ""
            user_question = prompt
            
            # Bóc tách chuỗi dữ liệu "Context:" được LightRAG chuẩn bị sẵn trong biến prompt
            lowered_prompt = prompt.lower()
            if "context:" in lowered_prompt:
                if "user question:" in lowered_prompt:
                    # Cấu trúc thông thường của LightRAG: Context:\n...\n\nUser Question:...
                    parts = prompt.split("User Question:")
                    context_data = parts[0].replace("Context:", "").strip()
                    user_question = parts[1].strip() if len(parts) > 1 else prompt
                else:
                    # Cấu trúc dự phòng nếu không tìm thấy chuỗi "User Question:"
                    context_data = prompt.replace("Context:", "").strip()
                    user_question = "Hãy tư vấn các sản phẩm dựa trên thông tin được cung cấp."
            
            # Nếu tìm thấy dữ liệu tri thức được bóc tách từ DB
            if context_data and system_prompt:
                # Đắp trực tiếp dữ liệu tri thức thô vào vị trí NGUỒN THÔNG TIN CỐT LÕI trong SALES_PROMPT của anh
                modified_system = system_prompt.replace(
                    "Bạn sẽ được cung cấp một đoạn dữ liệu (Context) chứa danh mục sản phẩm, giá cả và đặc tính.",
                    f"Dưới đây là đoạn dữ liệu (Context) thực tế của cửa hàng chứa danh mục sản phẩm, giá cả và đặc tính:\n{context_data}"
                )
                
                # Ép chặt kịch bản tin nhắn gửi lên API của Groq
                kwargs["messages"] = [
