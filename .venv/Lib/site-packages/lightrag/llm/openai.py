...
async def openai_complete(
    ...
    **kwargs,
    ...
    if "llama" in model_name.lower():
        ...
        # 1. Sửa lỗi trích xuất từ khóa (Giữ nguyên)
        if keyword_extraction:
            keyword_extraction = False 
            kwargs["response_format"] = {"type": "json_object"}
            prompt += (
                ...
                "\nDo not include any explanation or markdown format."
            )
            
        # 2. CAN THIỆP ĐẬP HỘP MESSAGES ĐỂ ÉP LLAMA LUÔN ĐỌC DỮ LIỆU PDF/ĐỒ THỊ
        elif system_prompt and "context:" in prompt.lower():
            # Trích xuất dữ liệu thô từ biến prompt của LightRAG
            ...
            # Đắp cấu trúc tin nhắn siêu nghiêm ngặt gửi thẳng vào API, đè lên biến messages mặc định
            kwargs["messages"] = [
                {"role": "system", "content": system_prompt},
                {
                    "role": "user", 
                    "content": (
                        "=== NGUỒN THÔNG TIN CỐT LÕI (CONTEXT) ===\n"
                        f"{context_data}\n"
                        "=========================================\n"
                        "LƯU Ý: Đọc kỹ nguồn thông tin trên để trả lời trực tiếp câu hỏi dưới đây."
                    )
                },
                {
                    "role": "user", 
                    "content": f"Câu hỏi của khách hàng: {user_question}\n\n(Yêu cầu: Khẳng định Có/Không hoặc Được/Không Được ngay câu đầu tiên, sau đó giải thích ngắn gọn dựa vào Context)."
                }
            ]
