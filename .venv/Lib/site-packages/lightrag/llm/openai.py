...
async def openai_complete(
    ...
    **kwargs,
    ...
    # MẶC ĐỊNH BẮT ĐƯỜNG TRUYỀN SANG GROQ NẾU DÙNG LLAMA CỦA GROQ
    if "llama" in model_name.lower():
        kwargs["base_url"] = "https://api.groq.com/openai/v1"
        kwargs["api_key"] = os.environ.get("OPENAI_API_KEY") # Lấy Key gsk_ của bạn
        
        # 1. SỬA LỖI JSON_SCHEMA CHO KHÂU TRÍCH XUẤT TỪ KHÓA
        if keyword_extraction:
            keyword_extraction = False 
            kwargs["response_format"] = {"type": "json_object"}
            # Bổ sung chỉ thị cứng nhắc để Llama 3.3 trả về đúng cấu trúc JSON phẳng của LightRAG
            prompt += (
                ...
                "\nDo not include any explanation or markdown block outside the JSON."
            )
            
        # 2. CHUẨN HÓA ĐỊNH DẠNG CONTEXT DÀNH RIÊNG CHO LLAMA 3.3
        # Khi LightRAG trả kết quả truy vấn, biến `prompt` sẽ có dạng "Context:\n...\n\nUser Question:..."
        elif system_prompt and "context:" in prompt.lower():
            # Tách khối dữ liệu Tri thức và câu hỏi thực tế của khách hàng ra
            if "user question:" in prompt.lower():
                ...
                # Phòng trường hợp cấu trúc chuỗi của LightRAG thay đổi nhẹ ở các mode khác nhau
                context_data = prompt.replace("Context:", "").strip()
                user_question = prompt
                
            # Tạo một khối Context tường minh đặt tên là "Context" nằm ngay trước câu hỏi của user
            # Giúp tầng Attention của Llama 3.3 ánh xạ trực tiếp sang câu "Bạn sẽ được cung cấp một đoạn dữ liệu (Context)" trong System Prompt cũ của bạn
            prompt = (
                f"=== ĐOẠN DỮ LIỆU (CONTEXT) ===\n{context_data}\n\n"
                f"Câu hỏi của khách hàng: {user_question}"
            )
