...
async def openai_complete(
    ...
    **kwargs,
    ...
    if "llama" in model_name.lower():
        ...
        # CHẶNG 1: ÉP LAMA TRÍCH XUẤT TỪ KHÓA CHUẨN
        if keyword_extraction:
            keyword_extraction = False 
            kwargs["response_format"] = {"type": "json_object"}
            prompt += (
                ...
                "\nFocus on product categories, brands, or specific pets mentioned."
                "\nDo not include any explanation or markdown formatting outside the JSON."
            )
            
        # CHẶNG 2: KHÓA CHẶT TƯ DUY - TUYỆT ĐỐI KHÔNG CHO BỊA ĐẶT CHIM, CÁ, DỊCH VỤ
        else:
            context_data = ""
            ...
            if context_data and system_prompt:
                # Tạo một bộ quy tắc System Prompt cực kỳ sắt đá đè lên Sales_Prompt cũ
                strict_system_prompt = (
                    f"{system_prompt}\n\n"
                    "=== NGUỒN THÔNG TIN CỐT LÕI (DỮ LIỆU THỰC TẾ) ===\n"
                    f"{context_data}\n"
                    "================================================\n"
                    "🚨 LỆNH BẮT BUỘC KHẨN CẤP TỪ HỆ THỐNG:\n"
                    "1. Đoạn dữ liệu CONTEXT phía trên là TOÀN BỘ những gì cửa hàng đang có. "
                    "Nếu trong CONTEXT chỉ liệt kê 2 sản phẩm hoặc 1 loài vật, bạn CHỈ ĐƯỢC PHÉP trả lời đúng những thông tin đó.\n"
                    "2. TUYỆT ĐỐI KHÔNG ĐƯỢC TỰ SUY DIỄN, không được bịa thêm bất kỳ loài vật nào khác (ví dụ: chim, cá cảnh, chuột lang, hamster...) "
                    "hoặc bất kỳ dịch vụ nào (như tắm rửa, cắt tỉa lông) nếu không có chữ đó xuất hiện trong đoạn CONTEXT.\n"
                    "3. THà TRẢ LỜI ÍT VÀ THIẾU CÒN HƠN TRẢ LỜI SAI THỰC TẾ. Thấy sản phẩm nào trong dữ liệu thì liệt kê sản phẩm đó, không có thì bỏ qua."
                )
                
                kwargs["messages"] = [
                    {"role": "system", "content": strict_system_prompt},
                    {"role": "user", "content": f"Câu hỏi của khách: {user_question}"}
                ]
