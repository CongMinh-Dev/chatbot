...
async def _ollama_model_if_cache(
    ...
    **kwargs,
    ...
    try:
        ...
        # --- ĐOẠN SỬA ĐỔI NÂNG CAO ĐỂ ÉP IN CHỮ THEO THỜI GIAN THỰC ---
        if not stream:
            print("\n🚀 [LLM START GENERATING] -> ", end="", flush=True)
            
            # Ép Ollama kích hoạt chế độ stream=True
            response_stream = await ollama_client.chat(model=model, messages=messages, stream=True, **kwargs)
            model_response = ""
            
            # Đọc dữ liệu nôn ra từng chút một và in ngay lập tức
            async for chunk in response_stream:
                content = chunk["message"]["content"]
                model_response += content
                print(content, end="", flush=True)  # In từng ký tự/từ lên màn hình Colab
                
            print("\n🏁 [LLM END GENERATING]\n")
            return model_response
        else:
            # Nếu LightRAG chủ động yêu cầu stream, giữ nguyên luồng gốc của thư viện
            response = await ollama_client.chat(model=model, messages=messages, stream=True, **kwargs)
            async def inner():
                try:
...
@wrap_embedding_func_with_attrs(
    embedding_dim=768, # sửa độ dài vector phù hợp với file model của mình
    max_token_size=8192,
...
async def ollama_embed(
    texts: list[str],
    # sửa thành mô hình embed của mình
